...
export default function InquiryForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 max-w-2xl space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[#0D1B3E] flex items-center gap-2.5">
          <Building2 className="text-[#3B82F6]" size={26} />
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-3">
          <input
            ...
            maxLength={8}
            className="flex-1 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3B82F6] outline-none text-[#0D1B3E] font-medium bg-white"
          />
          <button
            type="button"
            onClick={handleAresLookup}
            disabled={isPending || !ico.trim()}
            className="px-5 py-3 bg-[#3B82F6] hover:bg-blue-600 text-white font-semibold rounded-lg shadow-sm transition-colors disabled:opacity-50 flex items-center gap-2 shrink-0"
          >
            {isPending ? (
              <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
        <div className="md:col-span-2 space-y-1.5">
          <label className="block text-sm font-semibold text-[#0D1B3E]">Název firmy / Jméno</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="např. Firma s.r.o."
            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3B82F6] outline-none text-gray-900 bg-white"
            required
          />
        </div>

        <div className="space-y-1.5">
          <label className="block text-sm font-semibold text-[#0D1B3E]">DIČ (nepovinné)</label>
          <input
            type="text"
            value={dic}
        ...
        <div className="space-y-1.5">
          <label className="block text-sm font-semibold text-[#0D1B3E]">PSČ</label>
          <input
            type="text"
            value={zip}
        ...
        <div className="md:col-span-2 space-y-1.5">
          <label className="block text-sm font-semibold text-[#0D1B3E]">Ulice a číslo popisné</label>
          <input
            type="text"
            value={street}
        ...
        <div className="md:col-span-2 space-y-1.5">
          <label className="block text-sm font-semibold text-[#0D1B3E]">Město</label>
          <input
            type="text"
            value={city}
      ...
      <div className="space-y-1.5 pt-2">
        <label className="block text-sm font-semibold text-[#0D1B3E]">Poznámka k poptávce</label>
        <textarea
          value={note}
