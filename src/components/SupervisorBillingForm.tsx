...
export default function SupervisorBillingForm({ 
  ...
  return (
    <div className="mt-6 pt-6 border-t border-zinc-200 bg-zinc-50 rounded-xl p-6">
      <h4 className="text-lg font-black text-[#000000] mb-4 flex items-center gap-2">
        <Calculator className="text-[#FF4F00]" /> Vyúčtování a analýza spotřeby (Supervizor)
      </h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1">Stroj udělal {totalLifts} zdvihů = {totalKg.toFixed(1)} kg materiálu.</p>
          </div>
          ...
          <div>
            <label className="text-xs font-bold text-zinc-500 uppercase">Hodinová sazba za balení (Kč/h)</label>
            ...
            <p className="text-xs text-zinc-400 mt-1">Aplikátor balil {evidence.packingHours || 0} hodin.</p>
          </div>
        ...
        <div className="bg-[#000000] text-[#FEFEFA] p-6 rounded-xl shadow-inner flex flex-col justify-between">
          <div>
            <div className="flex justify-between border-b border-zinc-800 pb-2 mb-2 text-sm">
            ...
            <div className="flex justify-between border-b border-zinc-800 pb-2 mb-4 text-sm">
            ...
            <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800 mb-6">
              <div className="text-xs text-zinc-500 uppercase font-bold mb-1">Technologická analýza</div>
              ...
              {materialLossPercent > 20 && (
                <div className="text-xs text-red-400 mt-2 flex items-start gap-1">
                  ...
                  Pozor: Ztráta je nestandardně vysoká. Zkontrolujte tloušťku nebo aplikátora.
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            <div className="text-sm text-zinc-400 uppercase font-bold">Celkem k fakturaci</div>
            <div className="text-4xl font-black text-[#FF4F00] mb-4">
              {finalTotal.toLocaleString('cs-CZ')} Kč
            </div>
            <button 
              onClick={handleSave}
              disabled={isSaving}
              className="w-full py-3 bg-[#FEFEFA] text-[#000000] hover:bg-zinc-200 font-bold rounded-lg transition-colors flex items-center justify-center gap-2 cursor-pointer"
            >
              <Save size={18} />
              {isSaving ? 'Ukládám...' : (evidence.finalInvoiceTotal ? 'Aktualizovat vyúčtování' : 'Uložit a uzamknout vyúčtování')}
            </button>
          </div>
        </div>
      </div>
