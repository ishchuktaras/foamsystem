      ...
      <div className="relative w-full rounded-2xl bg-linear-to-r from-[#000000] to-[#1a1a1a] border border-zinc-800 p-6 md:p-10 text-[#FEFEFA] shadow-xl overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-zinc-400 text-sm md:text-lg leading-relaxed">
            {isApplicator 
              ? 'Seznam zakázek, které vám byly přiděleny k realizaci. Zde je váš technický deník (přístup do detailu zakázky pro realizaci na stavbě).'
              : 'Kompletní evidence klientských zakázek. Tvořte cenové nabídky, sledujte stav rozpracovaných projektů a kontrolujte technický deník od aplikátorů.'
            }
          </p>
        </div>
            ...
            {quotes.map((quote) => (
              <div key={quote.id} className="bg-[#FEFEFA] p-4 rounded-xl shadow-sm border border-zinc-200 flex flex-col gap-4 w-full">
                ...
                <div className="flex justify-between items-start border-b border-zinc-100 pb-3">
                  <div className="pr-2">
                    <Link href={isApplicator ? `/admin/quotes/${quote.id}/evidence` : `/admin/quotes/${quote.id}`} className="font-bold text-[#000000] text-lg leading-tight hover:text-[#FF4F00] transition-colors">
                      {quote.customerName}
                    </Link>
                ...
                <div className="pt-3 border-t border-zinc-100 flex flex-col gap-2">
                  {quote.status !== 'COMPLETED' && isApplicator && (
                    <Link 
                    ...
                    >
                      ...
                      Otevřít technický deník stavby
                    </Link>
                  )}
                  {!isApplicator && (
                    <div className="flex gap-2 mt-1">
                      <Link 
                        ...
                        className="w-full py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-xl text-sm transition-colors flex items-center justify-center cursor-pointer border border-blue-200"
                      >
                        Detail a Fakturace
                      </Link>
                      <Link 
                        href={`/admin/quotes/${quote.id}/edit`}
                        className="w-full py-2.5 bg-zinc-100 hover:bg-zinc-200 text-[#000000] font-bold rounded-xl text-sm transition-colors flex items-center justify-center cursor-pointer border border-zinc-200"
                      >
                        Upravit
                      </Link>
                  ...
                  {quotes.map((quote) => (
                    <tr key={quote.id} className="hover:bg-zinc-50 transition-colors">
                      <td className="py-4 px-6">
                        <Link href={isApplicator ? `/admin/quotes/${quote.id}/evidence` : `/admin/quotes/${quote.id}`} className="font-bold text-[#000000] text-base hover:text-[#FF4F00] transition-colors">
                          {quote.customerName}
                        </Link>
                      ...
                      <td className="py-4 px-6 text-right whitespace-nowrap flex justify-end items-center gap-2">
                        {quote.status !== 'COMPLETED' && isApplicator && (
                          ...
                          >
                            <ClipboardCheck size={16} /> Otevřít deník
                          </Link>
                        )}
                        {!isApplicator && (
                          <>
                            <Link 
                              href={`/admin/quotes/${quote.id}`}
                              className="px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 font-medium text-sm rounded-md transition-colors inline-block"
                            >
                              Detail
                            </Link>
                            <Link 
                              href={`/admin/quotes/${quote.id}/edit`}
                              className="px-3 py-1.5 bg-zinc-100 text-[#000000] border border-zinc-200 hover:bg-zinc-200 font-medium text-sm rounded-md transition-colors inline-block"
                            >
                              Upravit
                            </Link>
